...
(defun phps-mode-functions-around-delete-backward-char (old-function &rest arguments)
  "Call OLD-FUNCTION with ARGUMENTS and then shift indexes if the rest of the line is just white-space."
  (if (string= major-mode "phps-mode")
      (let ((old-pos (point)))
        (if (> old-pos 0)
            ;; (message "Running advice")
            (let* ((previous-char (buffer-substring-no-properties (1- old-pos) old-pos))
                  (looking-at-whitespace (string-match-p "[\ \n\t\r]*\n" previous-char))
                  (old-line-number (line-number-at-pos)))
