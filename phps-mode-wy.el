;;; phps-mode-wy.el --- Generated parser support file -*- lexical-binding: t -*-

;; Copyright (C) 2018-2020  Free Software Foundation, Inc.

;; Author: Christian Johansson <christianjohansson@Christians-MacBook-Air.local>
;; Created: 2020-07-05 09:48:29+0200
;; Keywords: syntax
;; X-RCS: $Id$
...
;; generated from the grammar file phps-mode-parser-grammar-raw.wy.
       ...
       (inner_statement
        ...
        ((T_HALT_COMPILER OPEN_PARENTHESIS CLOSE_PARENTHESIS SEMICOLON)
         (let
             ((r))
           (setq r nil)
           (ZEND_THROW_EXCEPTION phps-mode-parser--zend_ce_compile_error "__HALT_COMPILER() can only be used from the outermost scope" 0)
       ...
       (function_declaration_statement
        ((function returns_ref T_STRING backup_doc_comment OPEN_PARENTHESIS parameter_list CLOSE_PARENTHESIS return_type backup_fn_flags OPEN_CURLY_BRACKET inner_statement_list CLOSE_CURLY_BRACKET backup_fn_flags)
         (let
             ((r))
           (setq r
                 (ZEND_AST_CREATE_DECL 'phps-mode-parser--ZEND_AST_FUNC_DECL
                                       (logior $2 $13)
                                       $1 $4
                                       (ZEND_AST_GET_STR $3)
                                       $6 nil $11 $8))
           (CG 'phps-mode-parser--extra_fn_flags $9)
           r)))
       (is_reference
        ((%empty)
         (let
       ...
       (class_declaration_statement
        ...
        ((T_CLASS
         ...
         (let
             ((r))
           (setq r
                 (ZEND_AST_CREATE_DECL 'phps-mode-parser--ZEND_AST_CLASS 0 $2 $6
                                       (ZEND_AST_GET_STR $3)
                                       $4 $5 $8 nil))
           r)))
       (class_modifiers
       ...
       (trait_declaration_statement
        ((T_TRAIT
         ...
         (let
             ((r))
           (setq r
                 (ZEND_AST_CREATE_DECL 'phps-mode-parser--ZEND_AST_CLASS 'phps-mode-parser--ZEND_ACC_TRAIT $2 $4
                                       (ZEND_AST_GET_STR $3)
                                       nil nil $6 nil))
           r)))
       (interface_declaration_statement
        ((T_INTERFACE
         ...
         (let
             ((r))
           (setq r
                 (ZEND_AST_CREATE_DECL 'phps-mode-parser--ZEND_AST_CLASS 'phps-mode-parser--ZEND_ACC_INTERFACE $2 $5
                                       (ZEND_AST_GET_STR $3)
                                       nil $4 $7 nil))
           r)))
       (extends_from
       ...
       (class_statement
        ...
        ((method_modifiers function returns_ref identifier backup_doc_comment OPEN_PARENTHESIS parameter_list CLOSE_PARENTHESIS return_type backup_fn_flags method_body backup_fn_flags)
         (let
             ((r))
           (setq r
                 (ZEND_AST_CREATE_DECL 'phps-mode-parser--ZEND_AST_METHOD
                                       (logior $3
                                               (logior $1 $12))
                                       $2 $5
                                       (ZEND_AST_GET_STR $4)
                                       $7 nil $11 $9))
           (CG 'phps-mode-parser--extra_fn_flags $10)
           r)))
       (class_name_list
        ((class_name)
         (let
           ...
           (setq r
                 (wisent-raw-tag
                  (semantic-tag 'phps-mode-parser--ZEND_AST_PROP_ELEM $1 $3
                                (if $4
                                    (ZEND_AST_CREATE_ZVAL_FROM_STR $4)
                                  nil))))
           r)))
       (class_const_list
        ...
        ((T_CLASS
         ...
         (let
           ...
           (setq phps-mode-parser--decl
                 (ZEND_AST_CREATE_DECL 'phps-mode-parser--ZEND_AST_CLASS 'phps-mode-parser--ZEND_ACC_ANON_CLASS $2 $6 nil $4 $5 $8 nil))
           (setq r
                 (wisent-raw-tag
       ...
       (expr
        ...
        ((T_YIELD)
         (let
           ...
           (CG 'phps-mode-parser--extra_fn_flags
               (logior
       ...
       (inline_function
        ((function returns_ref backup_doc_comment OPEN_PARENTHESIS parameter_list CLOSE_PARENTHESIS lexical_vars return_type backup_fn_flags OPEN_CURLY_BRACKET inner_statement_list CLOSE_CURLY_BRACKET backup_fn_flags)
         (let
             ((r))
           (setq r
                 (ZEND_AST_CREATE_DECL 'phps-mode-parser--ZEND_AST_CLOSURE
                                       (logior $2 $13)
                                       $1 $3
                                       (ZEND_STRING_INIT "{closure}"
                                                         (-
                                                          (SIZEOF "{closure}")
                                                          1)
                                                         0)
        ...
        ((fn returns_ref OPEN_PARENTHESIS parameter_list CLOSE_PARENTHESIS return_type backup_doc_comment T_DOUBLE_ARROW backup_fn_flags backup_lex_pos expr backup_fn_flags)
         (let
             ((r))
           (setq r
                 (ZEND_AST_CREATE_DECL 'phps-mode-parser--ZEND_AST_ARROW_FUNC
                                       (logior $2 $12)
                                       $1 $7
                                       (ZEND_STRING_INIT "{closure}"
                                                         (-
       ...
       (backup_doc_comment
        ((%empty)
         (let
             ((r))
           (setq r
                 (CG phps-mode-parser--doc_comment))
           (CG 'phps-mode-parser--doc_comment nil)
           r)))
       (backup_fn_flags
        ((%empty)
         [PREC_ARROW_FUNCTION]
         (let
           ...
           (setq r
                 (CG phps-mode-parser--extra_fn_flags))
           (CG 'phps-mode-parser--extra_fn_flags 0)
           r)))
       (backup_lex_pos
        ((%empty)
         (let
             ((r))
           (setq r
                 (LANG_SCNG phps-mode-parser--yy_text))
           r)))
       (returns_ref
        ((%empty)
         (let
       ...
       (class_name
        ((T_STATIC)
         (let
           ...
           (setq r
                 (ZEND_AST_CREATE_ZVAL_EX zv 'phps-mode-parser--ZEND_NAME_NOT_FQ))
           r))
        ((name)
         (let
             ((r))
...
(defun phps-mode--install-parser ()
  ...
  (setq semantic-parser-name "LALR"
        ...
        semantic-debug-parser-source "phps-mode-parser-grammar-raw.wy"
