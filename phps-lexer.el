...
(defun phps-mode/lexer-get-point-data()
  ...
  (save-excursion
    ...
    (let ((position (point))
      ...
      (let ((data (list (list start-in-scripting start-brace-level start-parenthesis-level start-inline-function-level start-token-number start-in-doc-comment) (list end-in-scripting end-brace-level end-parenthesis-level end-inline-function-level end-token-number end-in-doc-comment))))
        ;; (message "data: %s" data)
        data))))
